...
| Building a `.conda` file locally | Done, and its own test section run |
| A git tag and a GitHub release | Done, `v1.0.0` |
| A pull request to the modular-community repository | Opened |

Publishing a package puts a name in a shared namespace and invites people to
depend on it. That is why every row above the last two had to be green
first, and why the recipe gate now checks the recipe against the commit it
pins rather than against the working tree. It did not, once, and the
consequence is in [The recipe gate](#the-recipe-gate).
...
- **every path the recipe's scripts read exists at that revision**
...
The emphasised one was added after it was needed. A recipe pins one commit
and its build script is edited in another, so the two drift and nothing
notices until somebody builds the package. That is exactly what happened
here. Shell completions were added to the build script long after the pinned
commit, the gate reported the recipe consistent because the SHA was a real
commit, and the package build died on `cp cli/completions/knap.bash`. The
gate now resolves every path the scripts name against the tree of the pinned
commit, and reproduces that failure in under a second rather than after a
twenty second build.
...
would report a clean result for a recipe it never read. The scripts
themselves are taken from the raw text rather than from that parser, because
what matters is what bash will actually run.
...
next to the check itself. Two of its cases used to hardcode the SHA that
happened to be in the recipe when they were written, so updating the recipe
for a release silently disarmed both. They now read the revision out of the
recipe, because a self test that stops testing when the file it tests is
...
This is the sequence 1.0.0 went through, kept as the procedure for the next
release rather than as a record of this one.
...
5. Commit those together, then set `source.rev` in the recipe to that
   commit's full SHA and commit again. The recipe pins the commit whose tree
   it builds, and the second commit changes only the recipe, which the
   package does not contain. `python scripts/check_recipe.py` refuses a
   revision that is not a commit here, and refuses one whose tree is missing
   a path the build script reads, so neither half of this step can be
   skipped.
6. Tag the release and push the tag with the commits.
7. Build the package locally and run its test section.
8. Publish the GitHub release from the tag.
9. Open a pull request against the modular-community repository adding
   `recipes/knap/recipe.yaml`.
10. Archive the tagged release through Zenodo for a citable DOI, which pairs
