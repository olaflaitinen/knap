...
1. [The two orderings that were not negotiable](#the-two-orderings-that-were-not-negotiable)
2. [How each claim was verified](#how-each-claim-was-verified)
3. [Paths the original layout did not list](#paths-the-original-layout-did-not-list)
...
## The two orderings that were not negotiable

A gate is not passed until its condition has actually been run and observed.
Reasoning that the code looks right does not pass a gate. That single rule
produced two orderings, and both are worth keeping in view because they are
the reason this library can make the claims it makes.

**Scalar before vectorised.** The scalar scanner had to be correct, with its
golden test passing, before any vectorised classifier was written. The scalar
path is not scaffolding to be discarded once something faster exists. It stays
permanently as the reference the vectorised path is differentially tested
against, which is why `tests/test_classifier_parity.mojo` exists and why both
builds are exercised in continuous integration.

**Parity before optimisation.** Byte identical agreement with the reference
implementation was established, and gated, before a single line was written
for speed. An optimisation is a change that is supposed to preserve
behaviour, and without a gate that can refuse it there is no way to
distinguish a fast implementation from a wrong one. When the merge path was
later rewritten and came out between 1.39 and 1.85 times faster, each of its
four changes was checked against 191762320 tokens before it was kept.

Two other orderings turned out to matter and were not planned.

**The five remaining encodings came last.** Adding them before the
verification machinery existed would have meant adding them on trust. Adding
them afterwards meant every one arrived through the same gates the first two
did, and the two defects they exposed were found by those gates rather than
by a user.

**Packaging and the Python bindings were kept independent.** One depended on
a capability nobody had confirmed Mojo 1.0.0 had. It has it, but designing as
though it might not meant the answer changed nothing else.

## How each claim was verified

**Every condition below was observed, not inferred.** They are grouped by
what they establish, newest first, and each names the file, the test or the
run that satisfies it. A condition whose evidence reads "by inspection" is
not satisfied.

### Counting, memory, and the caller facing helpers

A counting entry point and a memory measurement. Both were expected
...
Encode became between 1.39 and 1.85 times faster with byte identical
output, which took Knap past `tiktoken` on three of the four distinct
encode behaviours and level with it on the fourth. This is the work the
...
### The five remaining encodings

The total went from two to all seven. The real content of that work is that
adding them found two defects in code that had been passing every gate for
two encodings, and that both defects were in the assumptions rather than in
the algorithms.
...
### Packaging and the Python bindings

One packages the library for conda, the other exports it to Python as a
native extension. The second was the one that depended on a capability
nobody had verified Mojo 1.0.0 had. It has it.
...
### Vectorisation, caching and the benchmark suite

Two of the three produced qualified rather than triumphant results, and
fixing the benchmark corpus invalidated every performance claim the project
...
The fuzzer found the one real divergence class this project has had.
...
### BPE merge and encode

The merge rank table, the merge loop, and the public tokenizer API with
both encode entry points. Its gate, matching `tiktoken.encode` across the
corpus, passed first for two encodings over 80.5 million tokens and now
covers four distinct encode behaviours over 191.8 million.
...
The generated pattern constants, the Unicode tables, the UTF-8 decoder, the
scalar classifier, and the scanners. Its gate, matching the reference regex
over at least 100 MB of mixed text, passed for both patterns across 54.3
million pieces and now covers all three.

| Condition | Evidence |
| --- | --- |
| Pattern extracted, not transcribed | `scripts/extract_patterns.py`, with the committed constant checked for drift. |
| Unicode tables generated | Unicode 16.0.0, 2391 runs, verified against all 1114112 code points. Generated from 15.0.0 at first, which the differential fuzzer proved wrong. See [docs/UNICODE.md](UNICODE.md). |
...
### Vocabulary loading and decode

Vocabulary loading, the flat token store, decode over the full id space,
and the special token registry. Its gate, decoding every token id byte
identically against `tiktoken`, passed first for 300296 ids across two
vocabularies and now covers all 702463 across seven encodings. See
...
| Decode | Byte exact over both encodings, including the gaps, which raise. Later extended to all seven. |
...
| Suite under ASan | The whole suite as it stood then, 26 tests, passed with `--sanitize address`. It is now 19 files and 138 tests, and `sanitize.yml` still runs all of it. |

### The scaffold

The toolchain, the repository standard and continuous integration, before
any tokenizer code existed. Every figure in this table is what was true
then and is kept at that value deliberately, because the table records a
gate being passed on a date. Where the number has since moved, the current
value is given beside it.
...
| Unstable API inventory captured | 108 uses across 22 APIs, recorded in `docs/ARCHITECTURE.md`. It now reads 35957 uses across 86 APIs over 19 compiled targets, which measures code written rather than risk added. |
| Sanitizer job proven | `--sanitize address` builds and runs the suite clean. |

## Paths the original layout did not list

The brief that produced this repository asked for the whole directory tree to
be created up front with stub files, and separately forbade placeholders in
committed files. Those two instructions conflict, and the conflict was
resolved in favour of the no-placeholder rule: a file that cannot yet be
complete is absent rather than stubbed, because a stub that compiles and does
nothing passes every gate here while providing nothing.

Everything the layout asked for now exists, with one exception.
`src/knap/hf/tokenizer_json.mojo` was never written, and that is a decision
rather than a delay. It is in the deferred table below with the reasoning.

Some paths exist that the layout did not list. Every one is an addition
rather than a substitution, and none creates a parallel directory:

| Path | Why it exists |
| --- | --- |
| `tests/test_toolchain.mojo` | The layout had no slot for a toolchain smoke test. It deliberately imports nothing from `src/knap`, so a failure there always means the compiler rather than Knap. |
...
| `cli/` | The command line tool. It is an application rather than part of the library, so it sits outside `src/knap` and the library has no dependency on it. `cli/args.mojo` holds everything pure, which is what lets `tests/test_cli.mojo` check the parser without a vocabulary; `cli/tests/test_end_to_end.py` runs the built binary against the reference, because a parser test cannot tell you whether the numbers are right. |
| `scripts/selftest_gates.py` | The standard requires each gate to be observed failing on a planted violation. Doing that once by hand proves it once. This makes it repeatable and runs it in CI, so a gate that silently stops working is caught. |
...
| Hugging Face `tokenizer.json` loading | Deferred from the original layout, where it was listed as experimental. The format specifies its own pre-tokenizer rather than reusing either pattern Knap implements, so it needs a second parity corpus and a second reference implementation, which is a project in its own right rather than one file. The dependency evaluated for it, EmberJson, passed its acceptance criteria and the finding is kept in [docs/ARCHITECTURE.md](ARCHITECTURE.md) for whoever picks this up. |
...
| Whether Mojo can export an importable Python extension module is unverified | The bindings were designed around a flat C compatible surface so the `--emit shared-lib` fallback stayed open regardless of the answer. Resolved: it can. |
...
1.0.0 is tagged `v1.0.0` and published as a GitHub release. The conda recipe
in `conda.recipe/recipe.yaml` pins the commit that tag names, and
`scripts/check_recipe.py` refuses a recipe whose build script reads a path
that commit does not have. That check exists because it was once missing: the
recipe pinned a commit older than its own build script, and the package build
died on a file that had been added to the script months later.

Archiving the tag through Zenodo would produce a citable DOI, which pairs
naturally with the ORCID already recorded in `CITATION.cff`. That is the
author's decision to make and is not done unprompted.
